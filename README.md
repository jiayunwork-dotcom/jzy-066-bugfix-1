...
- 壁面热流（由壁面温度梯度导出，t>0）：
  ...
  ```
  q_w(t) = k(Tf−Tw)/(√(παt)) · 1/erf(λ)
  ```

  高斯因子只出现在**界面**梯度里：界面 Stefan 条件为
  `ρLf·ds/dt = q_w·e^{−λ²}`；壁面在 x=0，`e^{−x²/(4αt)}=1`。
  壁面取热 = 界面潜热 + 固相持续降温的显热，故 `q_w > ρLf·ds/dt`
  （预置冰层工况 1 h 为约 1432 W/m²，潜热部分约 1348 W/m²）。
...
- `s(4t) = 2·s(t)`（√t 律）；
- 只增大温差，同一时刻锋面更深、热流更大；
- 壁面热流由外部独立推导钉死：独立 Simpson 积分的 erf、独立二分的 λ
  算出解析壁面梯度（预置冰层 1 h ≈ 1432.4 W/m²），并对温度场做有限
  差分 Richardson 外推交叉核对，服务值与两者相对偏差分别 < 1e-9、1e-6；
- 全局能量闭合：q_w = ρLf·ds/dt + 固相显热累计速率，界面条件
  ρLf·ds/dt = q_w·e^{−λ²}；
- Lf→∞（Ste→0）锋面趋于停滞；
